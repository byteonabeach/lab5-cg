...
FrameContext Engine::beginFrame() {
    ...
    if (res == VK_ERROR_OUT_OF_DATE_KHR) {
        recreateSwapchain_();
        return {VK_NULL_HANDLE, 0, currentFrame, false}; // caller must call onResize
    }
...
void Engine::endFrame(const FrameContext& ctx) {
    ...
    if (vkQueueSubmit(graphicsQueue, 1, &si, inFlightFences[ctx.frameIndex]) != VK_SUCCESS)
    ...
    VkResult res = vkQueuePresentKHR(presentQueue, &pi);
...
TextureHandle Engine::registerTexture_(uint32_t w, uint32_t h,
                                       const unsigned char* pixels, VkDeviceSize byteSize) {
    ...
    transitionLayout(t.image, VK_FORMAT_R8G8B8A8_SRGB,
    ...
    t.view = createImageView(t.image, VK_FORMAT_R8G8B8A8_SRGB, VK_IMAGE_ASPECT_COLOR_BIT);
    ...
    // Allocate descriptor set for this texture (set 1, binding 0)
    VkDescriptorSetAllocateInfo ai{VK_STRUCTURE_TYPE_DESCRIPTOR_SET_ALLOCATE_INFO};
    ai.descriptorPool     = materialPool;
    ai.descriptorSetCount = 1;
    ai.pSetLayouts        = &materialLayout;
    if (vkAllocateDescriptorSets(device, &ai, &t.set) != VK_SUCCESS)
...
TextureHandle Engine::loadTexture(const std::string& path) {
    ...
    if (!pixels) {
        std::cerr << "[WARN] Cannot load: " << path << " — white fallback\n";
...
MeshHandle Engine::createMesh(const std::vector<Vertex>& verts,
                               const std::vector<uint32_t>& indices) {
    MeshRes m;
    m.indexCount = (uint32_t)indices.size();

    auto upload = [&](VkBufferUsageFlags usage, const void* src, VkDeviceSize sz,
    ...
    upload(VK_BUFFER_USAGE_VERTEX_BUFFER_BIT, verts.data(),
           sizeof(Vertex)*verts.size(), m.vb, m.vm);
    upload(VK_BUFFER_USAGE_INDEX_BUFFER_BIT, indices.data(),
           sizeof(uint32_t)*indices.size(), m.ib, m.im);
...
void Engine::recreateSwapchain_() {
    int w = 0, h = 0;
    while (w == 0 || h == 0) {
...
void Engine::createImage(uint32_t w, uint32_t h, VkFormat fmt,
...
void Engine::transitionLayout(VkImage img, VkFormat fmt,
...
VkImageView Engine::createImageView(VkImage img, VkFormat fmt,
