...
void GBuffer::createAttachments_(Engine& engine) {
    constexpr VkFormat fmts[NUM_ATTACHMENTS] = { FORMAT_POSITION, FORMAT_NORMAL, FORMAT_ALBEDO };
    ...
    engine.createImage(extent.width, extent.height, 1, depthFmt, VK_IMAGE_TILING_OPTIMAL, VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, depthImage, depthMemory);
...
void GBuffer::createRenderPass_(Engine& engine) {
    ...
    atts[3].format = depthFmt; atts[3].samples = VK_SAMPLE_COUNT_1_BIT; atts[3].loadOp = VK_ATTACHMENT_LOAD_OP_CLEAR; atts[3].storeOp = VK_ATTACHMENT_STORE_OP_DONT_CARE;
    atts[3].stencilLoadOp = VK_ATTACHMENT_LOAD_OP_DONT_CARE; atts[3].stencilStoreOp = VK_ATTACHMENT_STORE_OP_DONT_CARE;
    atts[3].initialLayout = VK_IMAGE_LAYOUT_DEPTH_STENCIL_ATTACHMENT_OPTIMAL; atts[3].finalLayout = VK_IMAGE_LAYOUT_DEPTH_STENCIL_ATTACHMENT_OPTIMAL;
    ...
    deps[1].srcSubpass = 0; deps[1].dstSubpass = VK_SUBPASS_EXTERNAL; deps[1].srcStageMask = VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT;
    deps[1].dstStageMask = VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT; deps[1].srcAccessMask = VK_ACCESS_COLOR_ATTACHMENT_WRITE_BIT;
...
void GBuffer::createFramebuffer_(Engine& engine) {
    std::array<VkImageView, 4> attachments = {views[0], views[1], views[2], depthView};
